(() => {
  ...
  const animate = ({targets, translateX, translateY, scale, opacity, width, duration = 400, easing = easings.outCubic, complete}) => {
  ...
  const sections = ["#about", "#projects", "#skills", "#contact"].map((id) => document.querySelector(id)).filter(Boolean);
  const spy = () => {
  ...
  const revealNow = (el) => {
    ...
    animate({targets: el, opacity: [0, 1], translateY: [12, 0], duration: 650, easing: easings.outCubic});
  };
  ...
  const PROJECTS = {
    dms: {
      ...
      subtitle: "End-to-end donation tracking with production-grade infrastructure.",
      pills: ["Node.js", "Express", "TypeScript", "Prisma", "PostgreSQL", "React", "Docker"],
      links: [{label: "GitHub Repository", url: "https://github.com/ManugaHewa/DonationManagamentSystem-DMS-/tree/Test"}],
      bullets: ["TypeScript monorepo ensuring type safety across API and UI", "Prisma ORM with PostgreSQL for reliable data modeling", "Admin dashboard with complex filtering, search, and reporting", "Dockerized deployment with CI/CD pipeline", "Full test coverage and error handling"],
    },
    exercise: {
      title: "Exercise Prescription App",
      subtitle: "Cross-platform clinician/patient app for personalized therapy.",
      pills: ["React Native", "TypeScript", "Node", "Express", "PostgreSQL", "GCP", "Docker"],
      links: [{label: "Frontend", url: "https://github.com/ManugaHewa/Exercise-Prescription-App_CAPSTONE/tree/main/Exercise-Prescription-App-Front-End-main"}, {label: "Backend", url: "https://github.com/ManugaHewa/Exercise-Prescription-App_CAPSTONE/tree/main/Exercise-Prescription-App-Back-End-main"}],
      bullets: ["Dual-role app: clinician assignment flows and patient progress tracking", "Real-time sync with offline capability for medical workflows", "RESTful APIs with proper validation and error recovery", "Microservices architecture on GCP for scalability", "Comprehensive testing suite for HIPAA-adjacent scenarios"],
    },
    alice: {
      title: "Alice in Brussels",
      subtitle: "Interactive exhibition with focus on performance and accessibility.",
      pills: ["HTML5", "CSS3", "JavaScript", "Bootstrap", "A11y"],
      links: [],
      bullets: ["Immersive multimedia experience with smooth 60fps interactions", "Interactive overlay system for exploration-driven navigation", "WCAG 2.1 AA accessibility standards throughout", "Optimized assets and responsive images for fast load times", "Keyboard navigation and semantic HTML for all interactions"],
    },
  };
  const openModal = (key, sourceEl) => {
    const p = PROJECTS[key];
    if (!p) return;
    modalBody.innerHTML = `<h3>${p.title}</h3><p style="color:var(--muted2); font-size:14px;">${p.subtitle}</p><div class="meta-row">${p.pills.map((t) => `<span class="meta-pill">${t}</span>`).join("")}</div>${p.links.length ? `<p style="margin:16px 0 0;">${p.links.map((l) => `<a class="link" href="${l.url}" target="_blank" rel="noreferrer">${l.label} -></a>`).join(" | ")}</p>` : ""}<h3 style="margin-top:24px;">Key Highlights</h3><ul style="color:var(--muted); line-height:1.8;">${p.bullets.map((b) => `<li style="margin-bottom:10px;">${b}</li>`).join("")}</ul>`;
    modal.classList.add("is-open");
    modal.setAttribute("aria-hidden", "false");
    document.body.style.overflow = "hidden";
    if (!prefersReduced) {
      animate({targets: ".modal-card", opacity: [0, 1], translateY: [12, 0], scale: [0.98, 1], duration: 420, easing: easings.outExpo});
  ...
  const closeModal = () => {
    ...
    animate({targets: ".modal-card", opacity: [1, 0], translateY: [0, 10], scale: [1, 0.985], duration: 220, easing: easings.inCubic, complete: finish});
  ...
  if (skillsCanvas) {
    ...
    // distribute points on a sphere (golden angle) for a fuller, spherical shape
    const goldenAngle = Math.PI * (3 - Math.sqrt(5));
    const radius = 0.56;
    const basePoints = labels.map((label, i) => {
      const y = 1 - (i / (labels.length - 1)) * 2; // -1..1
    ...
    const buildLinks = () => {
      ...
      // build candidate edges among active nodes
      ...
      // pass 1: add closest edges with cap 5
      edges.forEach(({ i, j }) => {
        if (degrees[i] < 5 && degrees[j] < 5) add(i, j);
      });

      // pass 2: ensure min 4 for active nodes
      ...
      // light connections for inactive nodes (to avoid isolation)
      indices.forEach((idx) => {
    ...
    const draw = (t = 0) => {
      ...
      const positions = nodes.map((n, i) => {
        let x = n.x * scale;
        let y = (n.y + 0.03) * scale; // nudge downward a bit
      ...
      positions.forEach((p, idx) => {
        ...
        const bubble = ctx.createLinearGradient(p.x - wRect / 2, p.y - hRect / 2, p.x + wRect / 2, p.y + hRect / 2);
